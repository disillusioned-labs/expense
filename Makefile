...
# root). Override to reach a different instance:
#   make migrate-up DB_DSN=postgres://expense_app:devpassword@localhost:5433/expense?sslmode=disable
DB_DSN ?= postgres://expense_app:devpassword@localhost:5432/expense?sslmode=disable
...
LDFLAGS := -s -w 	-X github.com/disillusioned-labs/expense/internal/app.version=$(VERSION) 	-X github.com/disillusioned-labs/expense/internal/app.commit=$(COMMIT) 	-X github.com/disillusioned-labs/expense/internal/app.buildDate=$(BUILD_DATE)
...
sqlc: ## Regenerate type-safe query code (runs once db/queries has its first query)
	$(SQLC) generate

sqlc-diff: ## Fail if generated code is stale (same check CI will run)
	$(SQLC) diff
...
docker-build: ## Build the production application image
	docker build 		--build-arg VERSION=$(VERSION) 		--build-arg COMMIT=$(COMMIT) 		--build-arg BUILD_DATE=$(BUILD_DATE) 		-t expense:$(VERSION) 		-t expense:latest 		.
